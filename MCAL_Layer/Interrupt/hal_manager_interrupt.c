...
void __interrupt() InterruptManager(void){
    ...
    if((PIE2bits.CCP2IE == Interrupt_Enable) && (PIR2bits.CCP2IF == Interrupt_Occur)) CCP2_ISR();
    if((PIE1bits.TXIE == Interrupt_Enable) && (PIR1bits.TXIF == Interrupt_Occur)) EUSART_TX_ISR();
    if((PIE1bits.RCIE == Interrupt_Enable) && (PIR1bits.RCIF == Interrupt_Occur)) EUSART_RX_ISR();
    //if((INTCON3bits.INT2IE == Interrupt_Enable) && (INTCON3bits.INT2IF == Interrupt_Occur)) INT2_ISR();
    //if((INTCON3bits.INT2IE == Interrupt_Enable) && (INTCON3bits.INT2IF == Interrupt_Occur)) INT2_ISR();
    //if((INTCON3bits.INT2IE == Interrupt_Enable) && (INTCON3bits.INT2IF == Interrupt_Occur)) INT2_ISR();
}
